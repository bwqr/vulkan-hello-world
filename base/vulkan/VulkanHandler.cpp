...
void VulkanHandler::createFramebuffers() {
    ...
    attachments[1] = vDepthImage.imageView;
...
void VulkanHandler::resizeCleanup() {
    vkDestroyImageView(device.logicalDevice, vDepthImage.imageView, nullptr);
    vkDestroyImage(device.logicalDevice, vDepthImage.image, nullptr);
    vkFreeMemory(device.logicalDevice, vDepthImage.memory, nullptr);
...
void VulkanHandler::createTexture(VkDeviceSize imageSize, void *data, uint32_t width, uint32_t height,
    ...
    vTexture->width = width;
    vTexture->height = height;
    vTexture->imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
}

void VulkanHandler::createImage(uint32_t width, uint32_t height, VkFormat format, VkImageTiling tiling,
                                VkImageUsageFlags usage, VkMemoryPropertyFlags properties, VulkanImage *vImage) {
    ...
    VK_CHECK_RESULT(vkCreateImage(device.logicalDevice, &imageInfo, nullptr, &vImage->image))

    VkMemoryRequirements memRequirements;
    vkGetImageMemoryRequirements(device.logicalDevice, vImage->image, &memRequirements);
    ...
    VK_CHECK_RESULT(vkAllocateMemory(device.logicalDevice, &allocInfo, nullptr, &vImage->memory))

    vkBindImageMemory(device.logicalDevice, vImage->image, vImage->memory, 0);
}
...
void VulkanHandler::createDepthResources() {
    createImage(windowExtent.width, windowExtent.height, depthFormat, VK_IMAGE_TILING_OPTIMAL,
                VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT,
                VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, &vDepthImage);

    swapChain.createImageView(vDepthImage.image, depthFormat, VK_IMAGE_ASPECT_DEPTH_BIT, 1, &vDepthImage.imageView);
}

void VulkanHandler::findDepthFormat() {
...
VkFormat VulkanHandler::findSupportedFormat(const std::vector<VkFormat> &candidates, VkImageTiling tiling,
...
VkDeviceSize VulkanHandler::minAlignment(VkDeviceSize size) {
    VkDeviceSize minBufferAlignment = device.properties.limits.minUniformBufferOffsetAlignment;

    if (minBufferAlignment > 0) {
        return (size + minBufferAlignment - 1) & ~(minBufferAlignment - 1);
    }
